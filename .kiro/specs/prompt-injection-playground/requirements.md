...
提示注入遊樂場（Prompt Injection Playground）是一個教育性工具，讓開發者能夠安全地實驗和理解提示注入攻擊。系統提供兩種注入模式（直接注入和間接注入），並使用虛擬電商環境來模擬真實場景。系統會自動分析攻擊結果，幫助開發者了解提示注入的風險和嚴重程度。

## 術語表

- **Playground（遊樂場）**: 使用者進行提示注入測試的互動介面
...
**使用者故事：** 作為開發者，我想要在遊樂場和儀表板之間切換，以便我可以查看測試結果和虛擬數據。
