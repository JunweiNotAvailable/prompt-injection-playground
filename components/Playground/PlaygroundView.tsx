  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-8">
      <div className="mb-8 flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 tracking-tight">提示注入測試</h1>
          <p className="mt-1 text-sm text-slate-500">在安全環境中測試 AI 助理的提示注入行為</p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {appState.currentMode === 'direct' && (
            <div className="relative" ref={dropdownRef}>
              <button
                ...
                className="inline-flex items-center gap-2 px-4 py-2 bg-white text-slate-700 text-sm font-medium rounded-lg border border-slate-200 hover:bg-slate-50 transition-colors shadow-sm"
              >
                <svg className="w-4 h-4 text-slate-500" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6l4 2m6-2a10 10 0 11-20 0 10 10 0 0120 0z" />
                </svg>
                切換會話
                <svg className={`w-3.5 h-3.5 text-slate-400 transition-transform ${sessionDropdownOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
              ...
              {sessionDropdownOpen && (
                <div className="absolute right-0 mt-2 w-80 bg-white border border-slate-200 rounded-xl shadow-lg z-20 overflow-hidden">
                  <div className="px-3 py-2 border-b border-slate-100 bg-slate-50">
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">近期直接注入會話</p>
                  </div>
                  <div className="max-h-72 overflow-y-auto">
                    {pastSessions.length === 0 ? (
                      <p className="px-4 py-3 text-sm text-slate-400">尚無歷史會話</p>
                    ) : (
                      pastSessions.map(s => {
                        ...
                        return (
                          <div
                            key={s.id}
                            className={`flex items-center gap-1 border-b border-slate-50 last:border-0 ${
                          ...
                          >
                            ...
                            >
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-1.5">
                                  <span className={`font-mono text-xs truncate ${isActive ? 'text-indigo-700 font-semibold' : 'text-slate-600'}`}>
                                    {s.id.slice(0, 8)}…
                                  </span>
                                  {isActive && (
                                    <span className="text-xs bg-indigo-100 text-indigo-600 px-1.5 py-0.5 rounded-full flex-shrink-0">目前</span>
                                  )}
                                </div>
                                <div className="flex items-center gap-2 mt-0.5">
                                  ...
                                  <span className={`text-xs ${(s.message_count ?? 0) > 0 ? 'text-emerald-600' : 'text-slate-400'}`}>
                                    {s.message_count ?? 0} 則訊息
                                  </span>
                                </div>
                              </div>
                            </button>
                            <button
                              ...
                              disabled={isDeleting}
                              title="刪除此會話"
                              className="p-2 mr-1 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-50 flex-shrink-0"
                            >
                              {isDeleting ? (
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-colors shadow-sm"
          >
      ...
      {loadingSession && (
        <div className="mb-4 flex items-center gap-2 px-4 py-2.5 bg-indigo-50 border border-indigo-200 rounded-lg text-sm text-indigo-700">
